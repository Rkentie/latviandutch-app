...
const Header: React.FC<HeaderProps> = ({ score, currentWordIndex, totalWords, onGoHome, t, streak }) => {
    ...
    <div className="w-full flex flex-col items-center pt-6 px-4 max-w-3xl mx-auto relative z-10">
      <div className="w-full flex justify-between items-center mb-6 gap-4">
        ...
        {streak !== undefined && streak > 0 && (
          <div className="flex items-center space-x-2 bg-white px-4 py-2 rounded-xl border-2 border-red-100 shadow-sm ml-2" title="Streak">
            <span className="text-red-500 text-xl">🔥</span>
            <span className="font-bold text-red-600 text-xl nunito-font">{streak}</span>
          </div>
        )}
      </div>
    </div>
